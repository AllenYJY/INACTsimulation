# -*- coding: utf-8 -*-
import requests, hashlib, urllib.parse, json
...
class Weather:
    ...
    def caculateAKSN(self):
        querystring = "location=%s&output=json&ak=%s"%(urllib.parse.quote_plus(self.city), self.ak)
        sn = '%s?%s%s'%(self.url,querystring,self.sk)
        sn = urllib.parse.quote_plus(sn)
